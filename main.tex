...
\langle a\rangle_m(\Delta)=\mathbb E_\xi\big[\sign(\bar s_T)\big]=\operatorname{erf}\!\Big(\frac{\sqrt T\,\Delta}{\sqrt2\,\sread}\Big),
\end{equation}
\begin{equation}
\langle a\rangle_m'(0)=\gain,\quad \pibar'(0)=\frac{\sqrt{T/2\pi}}{\sread}.
\end{equation}
...
\sum_j p_{ij}^{(m)}\,\langle a\rangle_m\big((\w_i-\w_j)\cdot\g_m\big)\approx \gain\,\g_m^{\!\top}\big((\Id_{\Nag\times\Nag}-\bm{P}^{(m)})\bm{\Omega}\big)_i,
\end{equation}
